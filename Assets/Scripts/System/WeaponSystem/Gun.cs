...
public class Gun : Weapon
{
    ...
    private void Start()
    {
        ...
        // per second.  Here, an actual ROF decimal value is calculated that can be used with timers.  计算在武器系统中使用的实际ROF。rateOfFire变量是为了方便用户使用而设计的——它表示每秒发射的子弹数。这里计算了一个实际的ROF十进制值，可以与计时器一起使用。
        if (rateOfFire != 0)
            actualROF = 1.0f / rateOfFire;
        else
            actualROF = 0.01f;

        // Initialize the current crosshair size variable to the starting value specified by the user   将当前准星大小变量初始化为用户指定的起始值
        currentCrosshairSize = startingCrosshairSize;

        // Make sure the fire timer starts at 0
        fireTimer = 0.0f;

        // Start the weapon off with a full magazine   用满弹匣启动武器
        ...
        // Initialize the bullet hole pools list   初始化弹孔池列表
        ...
        // Initialize the default bullet hole pools list   初始化默认的弹孔池列表
    ...
    private void Update()
    {
        ...
        // Calculate the current crosshair size.  This is what causes the crosshairs to grow and shrink dynamically while shooting 计算当前的准星大小。这就是导致准星在射击时动态增长和收缩的原因
...
//        print("玩家协程攻击");
//        yield return new WaitForSeconds(FireRate);//中断协程 停留攻击间隔时间
//    }
